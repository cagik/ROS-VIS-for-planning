...
bool Astar::plan(const State &start, const State &end, const Eigen::MatrixXi &map, KDTree &tree, vector<State>* resultPath)
{
    neighbor_ = {
            {-1, -1}, {-1, 0}, {-1, 1},
            {0, -1},            {0, 1},
    ...
    startPoint_ = make_pair(int(start.x), int(start.y));

    cout << "start : " << start.x << "  " << start.y << endl;
    cout << "end : " << end.x << "  " << end.y << endl;

    targetPoint_ = make_pair(int(end.x), int(end.y));
...
astarNode* Astar::FindPath()
{
    ...
    while(!OpenList.empty())
    {
        ...
        {   
            ...
            if(map_(x, y) == freeGrid || map_(x, y) == inOpenList)
            {
 ...
 * @return {*}
 */
void Astar::GetPath(astarNode* TailNode, vector<State>* path)
{
    PathList.clear();
    path->clear();
    ...
    {
        State tmp(PathList.back()->point.first, PathList.back()->point.second, 0);
        path->push_back(tmp);
        PathList.pop_back();
    }
