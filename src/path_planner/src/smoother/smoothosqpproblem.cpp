#include "smoother/smoothosqpproblem.h"
...
bool FemPosDeviationSqpOsqpInterface::Solve() {
  ...
  //参考点和box的size不一致，返回false
  if (ref_points_.size() != bounds_box_.size()) {
  ...
  // Sequential solution 序列化求解，当ftol < sqp_ftol_以及ctol < sqp_ctol_时返回true
  int pen_itr = 0;
  double ctol = 0.0;
...
void FemPosDeviationSqpOsqpInterface::CalculateAffineConstraint(
  ...
  for (int i = 0; i < 10; ++i) {
      const auto& ref_point_xy = ref_points_[i];
      (*upper_bounds)[i * 2] = ref_point_xy.first + bounds_box_[0][0];
      (*upper_bounds)[i * 2 + 1] = ref_point_xy.second + bounds_box_[0][1];
      (*lower_bounds)[i * 2] = ref_point_xy.first - bounds_box_[0][2];
      (*lower_bounds)[i * 2 + 1] = ref_point_xy.second - bounds_box_[0][3];
  }

  for (int i = 10; i < num_of_points_ - 10; ++i) {
      const auto& ref_point_xy = ref_points_[i];
      (*upper_bounds)[i * 2] = bounds_box_[i][2];
      (*upper_bounds)[i * 2 + 1] = bounds_box_[i][3];
      (*lower_bounds)[i * 2] = bounds_box_[i][0];
      (*lower_bounds)[i * 2 + 1] = bounds_box_[i][1];
  }

  for (int i = num_of_points_ - 10; i < num_of_points_; ++i) {
      const auto& ref_point_xy = ref_points_[i];
      (*upper_bounds)[i * 2] = ref_point_xy.first + bounds_box_[0][0];
      (*upper_bounds)[i * 2 + 1] = ref_point_xy.second + bounds_box_[0][1];
      (*lower_bounds)[i * 2] = ref_point_xy.first - bounds_box_[0][2];
...
/**
 * @description: 将points的数据转化为warm_start中的，用于求解。
 *               计算所有点的平均间距
 * @param {*}
 * @return {*}
 */
